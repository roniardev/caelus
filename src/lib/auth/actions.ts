...
export async function resendVerificationEmail(): Promise<{
  ...
  const lastSent = await db.query.emailVerificationCodes.findFirst({
    where: (table, { eq: eqFn }) => eqFn(table.userId, user.id),
...
export async function sendPasswordResetLink(
  ...
  try {
    const user = await db.query.users.findFirst({
      where: (table, { eq: eqFn }) => eqFn(table.email, parsed.data),
    });
    ...
    const verificationLink = `${env.NEXT_PUBLIC_APP_URL}/reset-password/${verificationToken}?email=${email}`;
...
export async function resetPassword(
  ...
  const dbToken = await db.transaction(async (tx) => {
    const item = await tx.query.passwordResetTokens.findFirst({
      where: (table, { eq: eqFn }) => eqFn(table.id, token),
    });
    if (item) {
      await tx
        .delete(passwordResetTokens)
